...
export default function TodoList(props) {
    const columnsBill = [
        ...
        },
        {
            title: '单项总价',
            dataIndex: 'itemPrice',
            render: (_, record) => { return record.sellPrice * record.usage }
        }
    ];
    ...
    const deleteRecord = (id) => {
        console.log(id);
        axios.delete(`http://localhost:7899/records/${id}`).then(
    ...
    const handleOk = () => {
        ...
        axios.patch(`http://localhost:7899/records/${editId}`, { bill: 2, billList: bill }).then(res => {
    ...
    return (
        <div>
            ...
            <Modal title="请确认金额并收款" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
                <Table
                    columns={columnsBill}
                    dataSource={bill}
                    pagination={false}
                    ...
                    summary={(pageData) => {
                        ...
                        pageData.forEach(({ sellPrice, usage }) => {
                            totalRepayment += sellPrice * usage;
                        });
                        return (
                            <>
                                <Table.Summary.Row >
