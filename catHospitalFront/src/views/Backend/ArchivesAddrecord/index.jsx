...
export default function ArchivesAddrecord() {
  ...
  useEffect(() => {
    axios.get('http://localhost:7899/staff/1/2').then(res => {
      setStaff(res.data)
    })
    ...
    if (recordInfo !== undefined) {
      //不能直接使用传过来的数据
      axios.get(`http://localhost:7899/records/${recordInfo.id}`).then(res => {
  ...
  const handleOk = () => {
    ...
    axios.post(`http://localhost:7899/records`, obj).then(res => {
  ...
  const renew = (reserve, bill) => {
    form.validateFields().then(res => {
      // console.log(reserve);
      axios.patch(`http://localhost:7899/records/${recordInfo.id}`,
        {
  ...
  const doneChange = async (value, name) => {
    ...
    const stockthing = await axios.get(`http://localhost:7899/inStocks/${treatment.inStockId}`)
    const usage = treatment.usage
    const stock = stockthing.data


    if (value) {//由未处理改成已处理
      if (stock.type === 1 && stock.amount < usage) {//库存不足
        ...
        form.validateFields().then(res => {
          ...
          axios.patch(`http://localhost:7899/records/${recordInfo.id}`,
            ...
            //库存信息更新
            axios.patch(`http://localhost:7899/inStocks/${treatment.inStockId}`,
              stock.type === 1 ? {
                amount: (stock.amount - usage),
                consumed: (stock.consumed + usage),
              } : { consumed: (stock.consumed + usage) }
            )
          })
        }).catch(res => {
  ...
  return (
    <div>
      ...
      <Modal title="请确认" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <p>是否将该病历提交给病历负责人</p>
      </Modal>
    </div >
  )
