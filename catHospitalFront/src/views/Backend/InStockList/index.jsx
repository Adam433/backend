...
export default function InStockList() {
  ...
  useEffect(() => {
    axios.get("http://localhost:7899/inStocks/type/1").then(res => {
  ...
  const handleOk = () => {
    if (delState === 'inStocks') {
      axios.delete(`http://localhost:7899/inStocks/${editItem.id}`)
    ...
    if (delState === 'inbound') {
      //删除单次入库数据、更新库存
      axios.delete(`http://localhost:7899/inbound/${editItem.id}`).then(res=>{
        axios.get(`http://localhost:7899/inStocks/${editItem.inStockId}`).then(res2=>{
          axios.patch(`http://localhost:7899/inStocks/${editItem.inStockId}`,{amount:res2.data.amount-editItem.amount})
        })
      })
      let arr = dataSource
      arr.forEach(item => {
  ...
  const addOk = () => {
    form.validateFields().then(res => {
      if (addOrEdit === 'add') {
        axios.post(`http://localhost:7899/inStocks/add`, { ...res, type: 1, consumed: 0, amount: 0, key: Date.now() }).then(
          res => {
            setDataSource([...dataSource, { ...res.data, inbound: [] }])
          })
      } else {
        axios.patch(`http://localhost:7899/inStocks/${editItem.id}`, res).then(res => {
          // console.log(res);
  ...
  const inboundOk = () => {
    formInbound.validateFields().then(res => {
      if (addOrEdit === 'add') {
        console.log(res);
        axios.post(`http://localhost:7899/inbound/add`, { ...res, key: Date.now(), staff: staff, inStockId : editItem.id, }).then(res2 => {
          console.log(res2);
          //更新页面
          ...
          setDataSource(arr)
          //更新单次入库数据时、更新库存
          axios.get(`http://localhost:7899/inStocks/${editItem.id}`).then(getIS=>{
            axios.patch(`http://localhost:7899/inStocks/${editItem.id}`,{amount:getIS.data.amount+res2.data.amount})
          })
        })
      } else {

        axios.patch(`http://localhost:7899/inbound/${editItem.id}`, res).then(resPatch => {
          setDataSource(dataSource.map(item => {
          ...
          axios.get(`http://localhost:7899/inStocks/${editItem.inStockId}`).then(res2=>{
            axios.patch(`http://localhost:7899/inStocks/${editItem.inStockId}`,{amount: res2.data.amount-editItem.amount+resPatch.data.amount})
          })
        })
      }
  ...
  return (
    <>
      <Table
        ...
        size="small"
      />
      <Modal title="删除确认" visible={isModalVisible} okButtonProps={{ type: '', danger: true }} onOk={handleOk} onCancel={handleCancel}>
        {delState === 'inStocks' ? <p>是否删除该项药品纪录？所有入库记录将保留</p> : <p>是否删除该项入库纪录</p>}
      </Modal>
      <Modal title={addOrEdit === 'add' ? "新增药品" : "药品信息编辑"} visible={addVisible} onOk={addOk} onCancel={addCancel}>
        <Form
          name="basic"
          form={form}
          labelCol={{
      ...
      <Modal title={addOrEdit === 'add' ? "新增入库" : "入库信息编辑"} visible={inboundVisible} onOk={inboundOk} onCancel={inboundCancel}>
        <Form
          name="inbound"
          form={formInbound}
          labelCol={{
