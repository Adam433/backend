...
export default function RightPage() {
  ...
  const handlechange = (checked, record) => {
    ...
    setUserinfo(obj)
    //将更新后的数据发送至json
    axios.patch(`http://localhost:7899/staff/${id}`,{
      showed:obj.showed
    })
  ...
  useEffect(() => {
    //模拟本地拿到用户rights 
    axios.get(`http://localhost:7899/staff/rights/${id}`).then(res => {
      // console.log(res.data);
      let arr = []
      //筛除根路径无权限的项,然后加载到表格中
      arr = res.data.rights.filter(child => {
    ...
    //获取用户信息：隐藏、显示那些页面
    axios.get(`http://localhost:7899/staff/${id}`).then(res => {
      // console.log(res.data);
