...
export default function ArchivesCats() {
  ...
  //删除宠物猫档案
  const handleDel = (id) => {
    axios.delete(`http://localhost:7899/cats/${id}`).then(res => {
  ...
  const addOk = () => {
    form.validateFields().then(res => {
      ...
      console.log(obj);
      if (addOrEdit === 'add') {
        axios.post(`http://localhost:7899/cats/add`, obj).then(res => {
          setData([obj, ...tempData])
          setTempData([obj, ...tempData])
        })
      } else {
        // console.log(obj);
        axios.patch(`http://localhost:7899/cats/${editItem.id}`, obj).then(res => {
          //对表格数据进行刷新
          const refresh = (data) => {
            return (
              data.map(item => {
