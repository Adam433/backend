...
export default function RightRole() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (dataIndex, record) => {
        return (
          <Tag color={color[record.id]} key={dataIndex}>
            {dataIndex}
          </Tag>
        )
      }
  ...
  const handleClick = (item) => {
    ...
    console.log(item.id);
    //复选框选中状态从json读取
    axios.get(`http://localhost:7899/roles/rights/${item.id}`).then(res => {
      console.log(res);
      let arr = []
      res.data.rights.forEach(child => {
  ...
  const handleOk = () => {
    ...
    // console.log(arr);
    axios.patch(`http://localhost:7899/roles/rights/${idEdit}`, arr).then(res => {
  ...
  useEffect(() => {
    axios.get('http://localhost:7899/roles/rights').then(res => {
  ...
  return (
    <div>
      ...
      <Modal title="请选择需要修改的权限" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <Tree
          checkable
          onExpand={onExpand}
