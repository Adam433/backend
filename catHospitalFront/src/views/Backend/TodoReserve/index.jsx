...
export default function TodoReserve(props) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:7899/records/${time}/${time + 1000 * 3600 * 24 * 7}/${doctor}`).then(res => {
  ...
  const children = forMap.map(item => {
    ...
    const handleClick = (item, record) => {
      ...
      if(token!==null){
        setAppointment(moment(time + item * 1000 * 3600 * 24 + (record.key + 0.1) * 1000 * 3600).valueOf())
        axios.get(`http://localhost:7899/cats`).then(res => {
  ...
  const handleOk = () => {
    ...
    axios.post(`http://localhost:7899/records`, {
      "catId": cat[0].id,
      "date": appointment,
      "staffId": doctor,
      "weight": '未称重',
  ...
  return (
    <div>
      ...
      <Table
        ...
        size="small"
      />
      <Modal title="请选择需要为哪只宠物猫预约" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
