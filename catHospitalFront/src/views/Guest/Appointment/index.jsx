...
export default function Appointment() {
  ...
  const handleNext = () => {
    form.validateFields().then(formValue => {
      axios.get(`http://localhost:7899/cats/phonename/${form.getFieldValue().phoneNumber}/${form.getFieldValue().nickname}`).then(res => {
        if (res.data.length === 0) { //为0就新建
          ...
          //数据进行格式整理
          let obj = { ...formValue, birthday: moment(formValue.birthday).valueOf(), vaccine: arr1, vermifuge: arr2, key: Date.now() }
          console.log(obj);
          axios.post(`http://localhost:7899/cats`, obj).then(res => {
            console.log(res);
  ...
  const handleFin = async () => {
    ...
    //如果没有档案、建立档案
    axios.post(`http://localhost:7899/records`, recordObj).then(res => {
      setCurrent(3)
    })
  ...
  //——————————————————————————————————————————————————表单中用到的函数
  const phoneOnBlur = () => {
    axios.get(`http://localhost:7899/cats/phone/${form.getFieldsValue().phoneNumber}`).then(res => {
  ...
  return (
    <div>
      ...
      <Modal title="预约情况确认" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <p>请问您确认预约信息：</p>
